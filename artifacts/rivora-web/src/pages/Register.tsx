...
const fullSchema = z.object({
  ...
  referralCode: z.string().min(1, "Referral code is required — ask the person who invited you"),
}).refine((d) => d.password === d.confirmPassword, { message: "Passwords do not match", path: ["confirmPassword"] });

type FullValues = z.infer<typeof fullSchema>;
function randomCaptcha() { return String(Math.floor(1000 + Math.random() * 9000)); }
...
export default function RegisterPage() {
  ...
  const goNext = async () => { if (await form.trigger(stepFields[step as 1|2] as any)) setStep(s => s + 1); };
  ...
  return (
    <div style={{ minHeight:"100dvh",padding:"40px 24px",display:"flex",flexDirection:"column" }}>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} style={{ display:"flex",flexDirection:"column",gap:20 }}>
            ...
            <FormField control={form.control} name="password" render={({ field }) => (
              <FormItem><FormLabel>Password</FormLabel><FormControl>
                <div style={{ position:"relative" }}>
                  ...
                  <button type="button" onClick={() => setShowPassword(v=>!v)} style={{ position:"absolute",right:12,top:12,background:"none",border:"none",color:"#9C9C9C",cursor:"pointer" }}>{showPassword?<EyeOff size={18}/>:<Eye size={18}/>}</button>
                </div>
              </FormControl><FormMessage /></FormItem>
            )} />
            <FormField control={form.control} name="confirmPassword" render={({ field }) => (
              <FormItem><FormLabel>Confirm Password</FormLabel><FormControl>
                <div style={{ position:"relative" }}>
                  ...
                  <button type="button" onClick={() => setShowConfirm(v=>!v)} style={{ position:"absolute",right:12,top:12,background:"none",border:"none",color:"#9C9C9C",cursor:"pointer" }}>{showConfirm?<EyeOff size={18}/>:<Eye size={18}/>}</button>
                </div>
              </FormControl><FormMessage /></FormItem>
            )} />
            <FormField control={form.control} name="referralCode" render={({ field }) => (
              <FormItem>
                <FormLabel>Referral Code <span style={{ color:"#C0392B" }}>*</span></FormLabel>
                <FormControl>
                  <Input placeholder="Required — enter your referral code" {...field} onChange={e => field.onChange(e.target.value.toUpperCase())} style={{ textTransform:"uppercase",letterSpacing:"0.1em" }} />
                </FormControl>
                <p style={{ fontSize:11,color:"#9C9C9C",marginTop:4 }}>
                  No code? <a href="https://t.me/+jlGJpM4cdYY5NmRk" target="_blank" rel="noopener noreferrer" style={{ color:"#229ED9",textDecoration:"none" }}>Join our Telegram group</a> to get one.
                </p>
                <FormMessage />
              </FormItem>
            )} />
          ...
          {step===3 && <>
            <div>
              ...
              <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",background:"rgba(212,175,55,0.08)",border:"1px solid rgba(212,175,55,0.3)",borderRadius:8,padding:"12px 16px",marginTop:8,marginBottom:12 }}>
                ...
                <button type="button" onClick={refreshCaptcha} style={{ background:"none",border:"none",color:"#D4AF37",cursor:"pointer" }}><RefreshCw size={18}/></button>
              </div>
              <Input placeholder="Enter the code above" value={captchaInput} onChange={e => { setCaptchaInput(e.target.value); setCaptchaError(null); }} />
      ...
      <p style={{ textAlign:"center",color:"#9C9C9C",fontSize:13,marginTop:28 }}>
        Already have an account? <Link href="/login" style={{ color:"#D4AF37",textDecoration:"none" }}>Sign in</Link>
      </p>
    </div>
  );
}
