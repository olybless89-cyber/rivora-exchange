...
import type { DepositRequest, User } from "@workspace/api-client-react";
  ...
  return (
    <AdminLayout title="Deposit Requests">
      ...
      {pendingCount > 0 && statusFilter === "pending" && (
        <div style={{ background: "rgba(245,158,11,0.08)", border: "1px solid rgba(245,158,11,0.25)", borderRadius: 10, padding: "10px 14px", marginBottom: 14 }}>
          <p style={{ fontSize: 13, color: "#f59e0b", margin: 0, fontWeight: 600 }}>
            ⏳ {pendingCount} deposit{pendingCount !== 1 ? "s" : ""} awaiting your approval
          </p>
        </div>
      )}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {allRequests.map((r: DepositRequest) => {
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                <div>
                  ...
                  <p style={{ fontSize: 11, color: "#9C9C9C", margin: "2px 0 0" }}>
                    {r.paymentMethod} · {format(new Date(r.createdAt), "MMM d, yyyy · h:mm a")}
                  </p>
                </div>
                <div style={{ textAlign: "right" }}>
                  ...
                  {r.reviewedAt && (
                    <p style={{ fontSize: 10, color: "#555", margin: "2px 0 0" }}>
                      Reviewed {format(new Date(r.reviewedAt), "MMM d · h:mm a")}
                    </p>
                  )}
                </div>
              </div>
              ...
              {r.status === "pending" && (
                <div style={{ display: "flex", gap: 8 }}>
                  <Button size="sm" className="flex-1"
                    ...
                    {updateRequest.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <><CheckCircle2 size={14} style={{ marginRight: 6 }} />Approve</>}
                  </Button>
